---
title: "월가아재 가치평가 흐름"
source_scope: "wsaj-video-evidence"
last_compiled_at: "2026-09-01"
evidence_ids:
  - wsaj-evidence-000001
  - wsaj-evidence-000002
  - wsaj-evidence-000003
  - wsaj-evidence-000004
  - wsaj-evidence-000005
  - wsaj-evidence-000006
  - wsaj-evidence-000007
  - wsaj-evidence-000008
  - wsaj-evidence-000009
  - wsaj-evidence-000010
  - wsaj-evidence-000011
  - wsaj-evidence-000012
  - wsaj-evidence-000013
  - wsaj-evidence-000014
  - wsaj-evidence-000025
claim_types:
  - direct_claim
  - inferred_principle
---

# 월가아재 가치평가 흐름

월가아재식 가치평가 흐름은 재무제표 확인에서 시작한다.
그 다음 내재가치와 상대가치를 구분하고, 배수의 구조와 분포를 본 뒤, 안전마진으로 최종 판단을 조정한다.

## 단계

| 단계 | 판단 | 증거 |
| --- | --- | --- |
| 재무제표 확인 | 리서치와 가치평가로 넘어가기 전에 재무제표 흐름을 본다 | `wsaj-evidence-000014` |
| 가치평가의 큰 틀 | 적정가치를 구하는 일을 내재가치와 상대가치 같은 도구로 나눈다 | `wsaj-evidence-000001` |
| 내재가치 | 현금흐름, 할인율, 성장률, 위험 같은 기업 고유 변수를 본다 | `wsaj-evidence-000003` |
| 상대가치 | 비교 가능한 주식과 배수를 통해 가격을 해석한다 | `wsaj-evidence-000004`, `wsaj-evidence-000010` |
| 배수 해석 | 평균 하나가 아니라 분포, 아웃라이어, 성장률, 위험을 함께 본다 | `wsaj-evidence-000005`, `wsaj-evidence-000007` |
| 안전마진 | 가치평가 뒤에 불확실성에 맞춰 여유분을 조절한다 | `wsaj-evidence-000012`, `wsaj-evidence-000013` |

## 배수별 주의점

PER은 주가를 주당순이익으로 나눈 값이다.
하지만 영상 근거에서는 PER을 단순 비율로 끝내지 않고 기대성장률, 배당성향, 위험과 연결해서 해석한다.
미국 주식에서는 GAAP 순이익과 non-GAAP 순이익의 차이도 확인해야 한다.

PBR은 ROE, 배당성향, 자기자본비용, 기대성장률과 연결해서 본다.
PSR은 일관성이 부족하므로 제한적으로만 사용한다.

## 적용 조건

하네스에서는 이 흐름을 `섹터 병목 확인 → 후보 선별 → 가치평가 → 안전마진 판단` 으로 연결한다.
특정 티커에서 시작해도 병목 확인은 건너뛰지 않는다.

`wsaj-evidence-000025` 는 직접 주장들을 묶은 가치평가 절차 추론이다.
따라서 이 문장을 쓸 때는 `wsaj-evidence-000001`, `wsaj-evidence-000003`, `wsaj-evidence-000010`, `wsaj-evidence-000013`, `wsaj-evidence-000014` 를 함께 제시한다.

## 오용 위험

PER, PBR, PSR 같은 배수는 숫자 하나로 외우면 안 된다.
유사기업을 대충 고르거나 평균만 보면 아웃라이어가 결론을 왜곡한다.
안전마진은 가치평가 전에 끼워 넣는 입력값이 아니라, 불확실성을 반영하는 최종 조정값이다.

## 관련 증거

- `wsaj-evidence-000001`: 가치평가의 의미와 도구 구분, `I4w_V9qUCXc` `79.4-116.3초`
- `wsaj-evidence-000003`: 내재가치 변수, `jS4thlOwR1U` `218.7-257.3초`
- `wsaj-evidence-000005`: PER 결정 변수, `7v8RppYkGcM` `615.6-756.7초`
- `wsaj-evidence-000009`: GAAP과 non-GAAP 구분, `w9WGecunB5I` `210.3-353.7초`
- `wsaj-evidence-000010`: 상대가치평가 순서, `BWbI_nhLTmg` `84.6-180.3초`
- `wsaj-evidence-000013`: 안전마진 조절, `Byn8et9uM3c` `137.1-221.0초`
