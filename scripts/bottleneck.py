...
# 자금 집중은 이미 오른 곳에 점수를 주는 축이므로 낮게 둔다.
# 초과수익의 기간 일관성은 정량 팩터로 중복 집계하지 않고 병목 실체 확인에서 본다.
#
# 수요 가속은 현재 데이터로 안정적으로 계산할 수 없어 뺐다.
# 수집 실측 제약은 skills/sector-bottleneck/references/valley-data.md에 둔다.
